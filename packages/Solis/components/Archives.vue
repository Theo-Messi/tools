<template>
  <div v-for="yearGroup in sortedArticlesByYear" :key="yearGroup[0].year">
    <div class="year">
      {{ yearGroup[0].year }}
    </div>
    <a :href="withBase(article.regularPath)" v-for="article in yearGroup" :key="article.regularPath" class="posts">
...
import { useYearSort, formatArticles, sortArticlesByTop } from '../types/functions'

const { theme } = useData()

// 对文章数据进行排序，置顶的文章会优先显示
const sortedArticlesByYear = computed(() => {
  const yearSortedData = useYearSort(theme.value.posts)

  return yearSortedData.map((yearGroup) => {
    const formattedArticles = formatArticles(yearGroup)
    return sortArticlesByTop(formattedArticles)
  })
})
</script>
